  ...
  return (
    <View>
      <Svg width={width} height={height}>
        <Line x1={padding} y1={padding + innerHeight} x2={padding + innerWidth} y2={padding + innerHeight} stroke={theme.colors.border} strokeWidth={1} />
        ...
        {coords.map((c, i) => (
          <Circle key={i} cx={c.x} cy={c.y} r={3.5} fill={theme.colors.accent} />
        ))}
      </Svg>
...
const makeStyles = (theme) => StyleSheet.create({
  ...
  axisLabel: { fontSize: 11, color: theme.colors.textFaint },
});
